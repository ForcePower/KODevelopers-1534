...
public:
	...
	void UpdateUserHWBAN(CUser *strUserID);
	...
	//visualdev
	void AccountBanned(std::string & strUserID );//banned account
	void AccountUnbanned(std::string & strUserID );//unbanned account
	void AccountIpBanned(std::string & strUserID );//ipbanned account
	void ItemOnline(uint32 nItemID,uint16 sCount);
	//end
