...
void CMagicProcess::MagicPacket(Packet & pkt, Unit * pCaster /*= nullptr*/)
{
	...
	if (pSkillCaster != nullptr){
		if(bOpcode == 0 
			&& pType4->bBuffType == BUFF_TYPE_SPEED 
			&& pSkillCaster->isNPC()){
				GrantType4Buff(pSkill, pType4, pSkillTarget, pSkillCaster, bIsRecastingSavedMagic);
				printf("%s monster iced by %s m_BSpeed %d\n",pSkillCaster->GetName().c_str(),pSkillTarget->GetName().c_str(),pSkillCaster->m_bSpeedAmount);
		}
	...
	if (pSkillTarget != nullptr){
		if(bOpcode == 1 && pType4->bBuffType == BUFF_TYPE_SPEED && pSkillTarget->isNPC()){
			RemoveType4Buff(pType4->bBuffType, pSkillTarget);
			printf("%s monster ice finished m_BSpeed %d\n",pSkillTarget->GetName().c_str(),pSkillTarget->m_bSpeedAmount);
		}}
...
bool CMagicProcess::UserRegionCheck(Unit * pSkillCaster, Unit * pSkillTarget, _MAGIC_TABLE * pSkill, int radius, short mousex /*= 0*/, short mousez /*= 0*/)
{
	...
	case MORAL_AREA_ALL:
		...
		if (TO_USER(pSkillCaster)->isInSafetyArea() || (pSkillTarget->isPlayer() && TO_USER(pSkillTarget)->isInSafetyArea()))
			return false;
		break;
...
bool CMagicProcess::GrantType4Buff(_MAGIC_TABLE * pSkill, _MAGIC_TYPE4 *pType, Unit * pCaster, Unit *pTarget, bool bIsRecastingSavedMagic /*= false*/)
{
	...
	if (!bIsRecastingSavedMagic
		...
		return false;
	/*
	if (pTarget->isPlayer() && pType->bBuffType==BUFF_TYPE_SPEED && pSkill->bSuccessRate < 100 && pSkill->bSuccessRate <= myrand(0, (100 - pSkill->bSuccessRate) * 10))
	return false;//
	*/
	if (pCaster == nullptr || nSkill == nullptr)
		return false;
	...
	case BUFF_TYPE_FREEZE:
		...
		pTarget->m_bSpeedAmount = pType->bSpeed;
#if defined(GAMESERVER)
		TO_USER(pTarget)->m_bAbnormalType = ABNORMAL_BLINKING;
		TO_USER(pTarget)->m_tBlinkExpiryTime = UNIXTIME + BLINK_TIME;
		TO_USER(pTarget)->m_bRegeneType = REGENE_ZONECHANGE;
		TO_USER(pTarget)->UpdateVisibility(INVIS_DISPEL_ON_ATTACK); // AI shouldn't see us
		TO_USER(pTarget)->m_bInvisibilityType = INVIS_NONE; // but players should. 
		TO_USER(pTarget)->StateChangeServerDirect(3, ABNORMAL_BLINKING);
#endif
		break;
	...
	case BUFF_TYPE_MAGE_ARMOR:
		pTarget->m_ReflectUsageSkill = nSkill->iNum;
		break;
