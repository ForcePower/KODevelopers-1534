...
void CUser::ClassChange(Packet & pkt, bool bFromClient /*= true */)
{
	...
	{
		if (g_pMain->Reskillmoney == 1 && g_pMain->Restatmoney == 1){
		result << uint8(CHANGE_MONEY_REQ) << uint32(0);
		Send(&result);
		return;
		}
...
void CUser::RecvSelectMsg(Packet & pkt)	// Receive menu reply from client.
{
	...
	if(isStoreOpen() || isMerchanting() || isTrading() || m_bMerchantStatex == 1 || szLuaFilename == "" || szLuaFilename.find("lua") == std::string::npos)
		return;
	//bu açýðý nasýl unuttun :D ne kanka bu :D görevi yaptýn diyelim teslim ettikten sonra sýnýrsýz ödül alabiliyosun
...
void CUser::NpcEvent(Packet & pkt)
{
	...
	if (isTrading() || m_bMerchantStatex == 1)
	{
	Packet resultmer;
	std::string bufferpro = string_format("Kural dýsý islem tespit edildi , tekrarý halinde uzaklastýrma alabilirsin."); 
	ChatPacket::Construct(&resultmer, 7, &bufferpro);
	Send(&resultmer);
	Packet resultmerxx;
	std::string bufferproxx = string_format("Kural dýsý islem tespit edildi , tekrarý halinde uzaklastýrma alabilirsin."); 
	ChatPacket::Construct(&resultmerxx, 13, &bufferproxx);
	Send(&resultmerxx);
	}
	else
	{
...
if (type == 3) 	// Move only (this is so useless mgame -- why not just handle it with the CUser::ItemMove(). Gah.)
	...
	if (type == 3)
	{
		...
		uint32 Rtime = m_sItemArray[SLOT_MAX+pos].nExpirationTime;
		...
		m_sItemArray[SLOT_MAX+destpos].nExpirationTime = Rtime;
	...
	if (type == 1)
	{	
		if(pTable->m_iBuyPrice > 200000 && count > 100)
		{
			errorCode = 4;
			goto fail_return;
		}
		...
		if (GetMap()->GetTariff() > 0)
			tariffTax = (pTable->m_iBuyPrice * GetMap()->GetTariff() / 100) ;
			...
			case ZONE_KARUS:
				if(pKingSystem == nullptr)
					break;
			pKingSystem->m_nTerritoryTax += (tariffTax * 25 / 100);
			pKingSystem->m_nNationalTreasury += (tariffTax * 75 / 100);
			InsertTaxUpEvent(ZONE_KARUS, (tariffTax * 25 / 100));
			InsertTaxUpEvent(ZONE_KARUS_ESLANT, (tariffTax * 75 / 100));
				break;
			...
			pKingSystem->m_nTerritoryTax += (tariffTax * 25 / 100);
			pKingSystem->m_nNationalTreasury += (tariffTax * 75 / 100);
			InsertTaxUpEvent(ZONE_ELMORAD, (tariffTax * 25 / 100));
			InsertTaxUpEvent(ZONE_ELMORAD_ESLANT, (tariffTax * 75 / 100));
				break;
		...
		if (pTable->m_iSellPrice != SellTypeFullPrice)
			transactionPrice = (((uint32)BuyPrice + tariffTax) * count);
		else
			...
			if(pNpc->m_iSellingGroup != (pTable->m_bSellingGroup*1000+1))
				if(pNpc->m_iSellingGroup != (pTable->m_bSellingGroup*1000+2))
					...
					string errorMessage = string_format(_T("NPC_ITEM_BUY uId-%s- nId-%s- P-%d- I-%d- Z-%d- X-%d- Y-%d- t-%d-"),
						GetName().c_str(),pNpc->GetName().c_str(),transactionPrice,itemid,GetZoneID(), uint16(GetX()), uint16(GetZ()),tariffTax);
	...
	else
	{
		...
		string errorMessage = string_format(_T("NPC_ITEM_SELL uId-%s- nId-%s- P-%d- I-%d- Z-%d- X-%d- Y-%d-"),
						GetName().c_str(),pNpc->GetName().c_str(),transactionPrice,itemid,GetZoneID(), uint16(GetX()), uint16(GetZ()));
		g_pMain->WriteTradeUserLogFile(errorMessage);
	}
