...
void CKnightsManager::PacketProcess(CUser *pUser, Packet & pkt)
{
	...
	case KNIGHTS_ALLY_CREATE:
		...
		break;
	case KNIGHTS_ALLY_REQ:// Kabul Ettým
		KnightsAllianceRequest(pUser, pkt);
		break;
	case KNIGHTS_ALLY_INSERT:// Yollar
...
void CKnightsManager::RecvUpdateKnights(CUser *pUser, Packet & pkt, uint8 command)
{
	...
	Packet result(WIZ_KNIGHTS_PROCESS, command);//error clan
...
bool CKnightsManager::CheckAlliance(CKnights * pMainClan, CKnights * pTargetClan)
{
	...
	if(pMainAlliance == nullptr || pTargetAlliance == nullptr)
		return false;

	if(pMainAlliance == pTargetAlliance)
		return true;
	else
		return false;
}
...
void CKnightsManager::KnightsAllianceCreate(CUser* pUser, Packet & pkt) {
	CUser *pTUser = g_pMain->GetUserPtr(pkt.read<uint16>());
	if (pUser == nullptr || pUser->isDead() || pTUser == nullptr || pTUser->isDead() || pTUser->GetNation() != pUser->GetNation() || pTUser->GetClanID() < 0 || !pTUser->isClanLeader())
		return;
	...
	if(pKnights->m_byFlag > 1 && pKnights->m_strChief == pUser->GetName()) {
		...
		result << uint8(1) << pUser->GetClanID();
		g_pMain->AddDatabaseRequest(result, pUser);
		pUser->Send(&result);
	}
}
...
void CKnightsManager::KnightsAllianceInsert(CUser* pUser, Packet & pkt)
{
	...
	do
	{
		...
		if (pAlliance == nullptr)
		{
			...
			if (pTknights != nullptr && pTknights->m_byFlag > 1 && pTknights->m_strChief == pUser->GetName() && pUser->isInGame())
			{
			KnightsAllianceCreate(pUser,pkt);
		...
		if ((pAlliance->sMercenaryClan_1 > 0 
		...
		pTUser->m_sAllyRequest = pUser->GetID();
...
void CKnightsManager::KnightsAllianceRequest(CUser* pUser, Packet & pkt) {
	...
	TargetClanLeaderID = pUser->m_sAllyRequest;

	CUser *pTUser = g_pMain->GetUserPtr(TargetClanLeaderID);
	
	

	if (pTUser == nullptr || pTUser->isDead() || pUser == nullptr || pUser->isDead() || pUser->GetNation() != pTUser->GetNation() || pUser->GetClanID() < 1 || !pUser->isClanLeader())
	{
		return;
	}
	...
	CKnights *pKnights = g_pMain->GetClanPtr(pUser->GetClanID());// ALLY GIREN
	CKnights *pTKnights = g_pMain->GetClanPtr(pTUser->GetClanID());// ALLY VEREN
	_KNIGHTS_ALLIANCE *pAlliance = g_pMain->GetAlliancePtr(pTUser->GetClanID());

	if (pAlliance == nullptr) {
		KnightsAllianceCreate(pTUser,pkt);
		g_pMain->ReloadKnightAndUserRanks();
		pAlliance = g_pMain->GetAlliancePtr(pTUser->m_bKnights);
		...
		if (pTKnights->m_byFlag > 1
			&& !pKnights->isInAlliance()
			&& pTKnights->m_strChief == pTUser->GetName()
			&& pAlliance->sMainAllianceKnights == pTKnights->GetAllianceID()
			...
			&& pAlliance->sSubAllianceKnights != pKnights->m_sIndex) {
				...
				result << uint8(1) << pTUser->GetClanID() << pUser->GetClanID() << pTKnights->m_sCape;
				g_pMain->AddDatabaseRequest(result, pTUser);
				pUser->Send(&result);

				
				std::string noticeText;
				g_pMain->GetServerResource(IDS_INSERT_KNIGHTS_ALLIANCE, &noticeText, pKnights->GetName().c_str());
...
void CKnightsManager::KnightsAlliancePunish(CUser* pUser, Packet & pkt) {	
	...
	if (pUser->isDead() || !pUser->isClanLeader() || !pUser->isInClan())
		return;


	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KNIGHTS_ALLY_PUNISH));
	uint16 PunishingClanID = pkt.read<uint16>();
	CKnights	*pTKnights = g_pMain->GetClanPtr(PunishingClanID),
	...
	if (pAlliance != nullptr) {
		...
		if (pUser->m_bKnights != pAlliance->sMainAllianceKnights)
			return;
		...
		if (pKnights->m_byFlag > 1
			...
			&& pAlliance != nullptr
			&& pTKnights->isInAlliance()
			&& pAlliance->sMainAllianceKnights != PunishingClanID
			...
			|| pAlliance->sSubAllianceKnights == pTKnights->m_sIndex)) {
				...
				if (pTUser != nullptr) {
					result << uint8(1) << pUser->GetClanID() << pTUser->GetClanID() << pTKnights->GetCapeID(pTKnights);
					g_pMain->AddDatabaseRequest(result, pUser);
					pTKnights->SendToRegion(&result);
				} else {
					result << uint8(1) << pUser->GetClanID() << PunishingClanID << pTKnights->GetCapeID(pTKnights);
					g_pMain->AddDatabaseRequest(result, pUser);
					pTKnights->SendToRegion(&result);
				}
