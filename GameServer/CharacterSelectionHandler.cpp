...
void CUser::SelCharToAgent(Packet & pkt)
{
	...
	{		
		Disconnect();
		printf("%s acc, %s nicked user deleted from sessionnmx.\n",strAccountID.c_str(),strUserID.c_str());
		result << uint8(0);
		Send(&result);
		return;
	}
...
void CUser::SelectCharacter(Packet & pkt)
{
	...
	result << GetZoneID() << GetSPosX() << GetSPosZ() << GetSPosY() << g_pMain->m_byOldVictory;//error
...
void CUser::SetLogInInfoToDB(uint8 bInit)
{
	...
	Packet result(WIZ_LOGIN_INFO);//error
	result	<< GetName() 
