...
void CUser::HandleStoreOpen(Packet & pkt)
{
	...
	if (isDead())
		...
		sErrorCode = -3;
	else if (isMerchanting() || m_bMerchantStatex == 1)
		sErrorCode = -4;
	...
	//sorun burada degil az dusuneyim kanka ben sorunu biliyorumda kelimeye dökemedim :D
	m_bStoreOpen = true;
	UserDataSaveToAgent();
	// hata varsa paket yolla yoksa yollama var bak burda
fail_return:
...
{ // sen var ya adamsýn :D
	// bi acigi kapatayim hazir gelmisken buraya
if(!m_bStoreOpen)
		return; 
...
void CUser::ReqLoadWebItemMall()
{
	...
	{
		_ITEM_DATA * pItem = GetItem(i);
		if (pItem->isSealed() || pItem->sRemainingRentalTime > 0)
			SendStackChange(pItem->nNum, pItem->sCount, pItem->sDuration, i - SLOT_MAX,pItem->sCount,pItem->sRemainingRentalTime);
	}
