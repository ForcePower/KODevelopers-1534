...
// Time (in seconds) between each skill request (-1 sec). that was 0.7f berat edited it to 0.1f
...
public:
	...
	*/
	...
	INLINE uint16 GetRequestAllyID() { return m_sAllyRequest; }
	...
	CUser * GetItemRoutingUser(uint32 nItemID, uint32 sCount, _LOOT_BUNDLE * pBundle);
	...
	bool isInBowl(uint16 X, uint16 Y);
	...
	COMMAND_HANDLER(HandleZoneCountCommand);
	...
	COMMAND_HANDLER(HandleBDWRegisterCommand);
...
public:
	...
	DECLARE_LUA_FUNCTION(GiveKC) {
		LUA_RETURN(LUA_GET_INSTANCE()->UpdateKC(
			LUA_ARG(uint32, 2)));
	}
	...
	DECLARE_LUA_FUNCTION(CheckWaiting) {
		...
		LUA_NO_RETURN(LUA_GET_INSTANCE()->TempleEventJoin()); // zone ID
	}
