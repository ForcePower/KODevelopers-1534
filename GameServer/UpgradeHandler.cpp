...
void CUser::ItemUpgrade(Packet & pkt, uint8 nUpgradeType)
{
	...
	{ // scoped lock to prevent race conditions
		...
		foreach_stlmap (itr, g_pMain->m_ItemUpgradeArray)
		{
			...
			if( (nItemID[1]== 379258000 && nItemID[2]== 379256000) || (nItemID[2]== 379258000 && nItemID[1]== 379256000)){
			...
			if ((nItemID[1] == 	379022000	||
				nItemID[1] == 	379030000	||
				nItemID[1] == 	379031000	||
				nItemID[1] == 	379032000	||
				nItemID[1] == 	379033000) && (proto->m_bSlot < 5 && proto->m_bSlot != 2))
				continue;
		...
		if(trina)
		{
			GenRate = (pUpgrade->sGenRate + 1000);
			if(GenRate>10000)
				GenRate=10000;
		}
		else 
...
void CUser::ItemUpgradeNotice(_ITEM_TABLE * pItem, uint8 UpgradeResult)
{
	...
	if (pItem->m_ItemType == 11 || pItem->m_ItemType == 12 || pItem->ItemClass == 8) 
	...
	if (bSendUpgradeNotice)
	{
		...
		if (UpgradeResult == 0){
			g_pMain->SendChat<SHOUT_CHAT>(sUpgradeNotice.c_str(),1);
			g_pMain->SendChat<SHOUT_CHAT>(sUpgradeNotice.c_str(),2);
		}
		if (UpgradeResult == 1){
			g_pMain->SendChat<KNIGHTS_CHAT>(sUpgradeNotice.c_str(),1);
			g_pMain->SendChat<KNIGHTS_CHAT>(sUpgradeNotice.c_str(),2);
		}
		...
		if (pExchange->bRandomFlag == 101 && resultOpCode == Success)
		{
			...
			if (resultOpCode == Success)
			{
				...
				if (pItem != nullptr)
				{
					...
					{
						sUpgradeNotice = string_format("%s has obtained of Chaotic generator '%s'.",GetName().c_str(),pItem->m_sName.c_str());
						g_pMain->SendUpgradeRebirth<FORCE_CHAT>(sUpgradeNotice.c_str());
					}
					///
				}
			}
