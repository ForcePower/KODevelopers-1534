...
CUser::CUser(uint16 socketID, SocketMgr *mgr) : KOSocket(socketID, mgr, -1, 81920, 16384), Unit(UnitPlayer)
{
}
...
void CUser::Initialize()
{
	...
	foreach(itr,g_pMain->m_characterNameMap)
	{

	if(itr->second != nullptr){
	if (itr->second->GetSocketID() == GetSocketID())
	{
	WillDelete = true;
	upperName = itr->first;
	}
	...
	03.03.2016 denenecek berat
	if (WillDelete)
	g_pMain->m_characterNameMap.erase(upperName);
	*/
}
...
void CUser::OnDisconnect()
{
	...
	if (isInGame())
	{
		...
		if (isInParty())
			...
			if (isInClan())
			{  //visualdev comentario
				/*CKnights *pKnights = g_pMain->GetClanPtr(GetClanID());
				if (pKnights != nullptr)
				{

					_KNIGHTS_ALLIANCE *pAlliance = g_pMain->GetAlliancePtr(GetClanID());
...
bool CUser::HandlePacket(Packet & pkt)
{
	...
	case WIZ_CONCURRENTUSER:
		CountConcurrentUser();
		break;
	case WIZ_DATASAVE://visualde liberado del comentario
		UserDataSaveToAgent();
		break;
...
void CUser::SendServerIndex()
{
	...
	if(isGM())
	{
		Packet pkt2(uint8(0xA1),uint8(0x01));
		Send(&pkt2);
	}
...
void CUser::LogOut()
{
	...
	UserInOut(INOUT_OUT);//visualdev agregado 21/01/2017
...
void CUser::SetZoneAbilityChange(uint16 sNewZone)
{
	...
	m_MagicTypeCooldownList.clear(); //error 
}
...
void CUser::SetSlotItemValue()
{
	...
	for (int i = 0; i < INVENTORY_TOTAL; i++)
	{
		...
		{
			_ITEM_TABLE *oItem = g_pMain->GetItemPtr(pItem->nNum);
			printf("%s users %s item deleted.\n",GetName().c_str(),oItem->m_sName.c_str());
			RobItem(i);
		}
	}
}
...
void CUser::RecvUserExp(Packet & pkt)
{
	...
	foreach (itr, partyUsers)
	{
		...
		if (iNpcExp > 0)
		{
			...
			if (iExp > (pUser->m_iMaxExp / 10) && pUser->m_iMaxExp > 1000)
...
{	
	...
	if ((GetLevel() < 6 && iExp < 0)
		|| (iExp < 0 && g_pMain->m_bIsPKServer == 1)
	...
	if (iExp > 0)
	{
		...
		if (!bIsBonusReward)
		{
			...
			if (GetPremiumProperty(PremiumExpPercent) > 0)// && !isDead()- fixed lose exp issue
...
{
	Packet result(WIZ_PARTY);// burda patladı //error
...
void CUser::SetUserAbility(bool bSendPacket /*= true*/)
{
	...
	if (p_TableCoefficient == nullptr || m_transformationType == TransformationSiege)
		return;
...
void CUser::ItemGet(Packet & pkt)
{
	...
	if (nItemID == ITEM_GOLD)
	{
		...
		else
		{
			...
			for (int i = 0; i < MAX_PARTY_USERS; i++)
			{
				...
				if (pUser->GetID() == GetID() && !isDead()){
					sumOfLevels += GetLevel();
					partyUsers.push_back(pUser);
					continue;
				}
	...
	else
	{
		...
		if(!isInParty())
		{
			...
			if (pTable->m_ItemType == 4 && pTable->ItemClass==3||pTable->m_ItemType == 4 && pTable->ItemClass==4||pTable->m_ItemType == 4 && pTable->ItemClass==8) 
				HasObtained = true;
			std::string sUpgradeNotice;
			if(HasObtained)  //drop notice
			{
				ShowEffect(490092);
				sUpgradeNotice = string_format("%s has obtained '%s'.",GetName().c_str(),pTable->m_sName.c_str());
				g_pMain->SendUpgradeRebirth<ALLIANCE_CHAT>(sUpgradeNotice.c_str());
			}
		}
		else
		...
		if (isInParty())
		{
			...
			{
				ShowEffect(490092);
				sUpgradeNotice = string_format("%s has obtained '%s'.",pReceiver->GetName().c_str(),pTable->m_sName.c_str());
				g_pMain->SendUpgradeRebirth<ALLIANCE_CHAT>(sUpgradeNotice.c_str());
			}
		}
	}
...
{
	if (!isInGame() || isGM() || GetFame() == COMMAND_CAPTAIN || isKing())
		return;
...
void CUser::SendNotice()
{
	...
	result.put(1, count); // replace the placeholdered line count //error
...
void CUser::AppendExtraNoticeData(Packet & pkt, uint8 & elementCount)
{
	...
	if (g_pMain->m_byExpEventAmount < 1000)
	{
		...
		string strname = "";
		ini.GetString("SERVER_NOTICE", "SERVER_NAME", "", names, false);
		ini.GetString("SERVER_NOTICE", "SERVER_MESSAGE", "", strname, false);

		AppendNoticeEntry(pkt, elementCount, strname.c_str(), names.c_str()); 
	}
	if (g_pMain->m_notice3 > 0)
	{
		g_pMain->GetServerResource(IDS_BAR1, &message, g_pMain->m_notice3);
		CIni ini(CONF_GAME_SERVER);
		string names1 = "";
		ini.GetString("SERVER_NOTICE", "SERVER_NOTICE1", "", names1, false);
		AppendNoticeEntry(pkt, elementCount, message.c_str(), names1.c_str()); 
	}
	if (g_pMain->m_notice2 > 0)
	{
		g_pMain->GetServerResource(IDS_BAR2, &message, g_pMain->m_notice2);
		CIni ini(CONF_GAME_SERVER);
		string names2 = "";
		ini.GetString("SERVER_NOTICE", "SERVER_NOTICE2", "", names2, false);
		AppendNoticeEntry(pkt, elementCount, message.c_str(), names2.c_str()); 
	}
	if (g_pMain->m_notice1 > 0)
	{
		g_pMain->GetServerResource(IDS_BAR3, &message, g_pMain->m_notice1);
		CIni ini(CONF_GAME_SERVER);
		string names3 = "";
		ini.GetString("SERVER_NOTICE", "SERVER_NOTICE3", "", names3, false);
		AppendNoticeEntry(pkt, elementCount, message.c_str(), names3.c_str()); 
	}
	...
	/* Packet result;
	std::string buffer = string_format("Welcome to "+g_pMain->m_bServerName+"!"+GetName()); 
	ChatPacket::Construct(&result, 7, &buffer);
	Send(&result);

	Packet resultl;
	std::string buffer2 = string_format("You can report any errors on facebook thanks"); 
	ChatPacket::Construct(&resultl, 7, &buffer2);
	Send(&resultl);
...
/*
	...
	Packet result4;
	std::string buffer4 = string_format("Any donation contact BlaZe | toda donación comunicarse con BlaZe"); 
	ChatPacket::Construct(&result4, 6, &buffer4);
	Send(&result4);
	*/ 

		Packet result2;
	std::string buffer2 = string_format("Welcome to Project: Knight Online Rebirth"); 
...
void CUser::LoyaltyDivide(int16 tid, uint16 bonusNP /*= 0*/)
{
	...
	if ((pTUser->GetNation() != GetNation() || GetZoneID() == ZONE_DELOS) || pTUser->GetZoneID() != GetZoneID()) // Different nations!!!
	{
	...
	for (int j = 0; j < MAX_PARTY_USERS; j++) // Distribute loyalty amongst party members.
	{
		...
		if (pUser->isAlive() && pUser->GetZoneID() == GetZoneID())
...
void CUser::ResetWindows()
{
	...
	if (isMerchanting() || m_bMerchantStatex == 1)
		MerchantClose();
...
CUser * CUser::GetItemRoutingUser(uint32 nItemID, uint32 sCount, _LOOT_BUNDLE * pBundle)
{
	if (!isInParty())
		return this;
...
void CUser::AllPointChange(bool bIsFree)
{
	...
	if (g_pMain->m_bIsPKServer)
		m_sPoints = 20 + (GetLevel() - 1) * 3;
	else
...
void CUser::SelectWarpList(Packet & pkt)
{
	...
	if (GetZoneID() == pWarp->sZone && pWarp->dwPay > 0 && hasCoins(pWarp->dwPay))//error
...
bool CUser::GetWarpList(int warp_group)
{
	...
	foreach (itr, warpList)
	{
		...
		if (g_pMain->isWarOpen() && g_pMain->m_byBattleOpen != CLAN_BATTLE
...
{
	if (jobgroupid <0)//para evitar caida de player
	{
		return false;
	}
	if (jobgroupid > 100) 
...
{
	g_pMain->Send_Region(pkt, GetMap(), GetRegionX(), GetRegionZ(), pExceptUser, GetEventRoom());
}
...
void CUser::OnDeath(Unit *pKiller)
{
	...
	if (pKiller != nullptr)
	{
		...
		if (pKiller->isNPC())
		{
			...
			if (GetNation() != GetZoneID() && GetZoneID() < ZONE_ELMORAD && g_pMain->isWarOpen())
				nExpLost = m_iMaxExp / 20;
		...
		else
		{
			...
			else
			{
				...
				{   
					Packet result(WIZ_EVENT, uint8(3));
					noticeType = DeathNoticeCoordinates;
					...
					g_pMain->Send_Zone(&resultmer, GetZoneID(), nullptr, Nation::ALL,GetEventRoom());

					if (g_pMain->pTempleEvent.ElmoDeathCount[GetEventRoom()] > 59 || g_pMain->pTempleEvent.KarusDeathCount[GetEventRoom()] > 59)
						g_pMain->TempleEventFinish(GetEventRoom());
				}
				else
				{
					...
					else
					{
						...
						else
						{
							...
							if (!GetMap()->isWarZone() && g_pMain->m_byBattleOpen != NATION_BATTLE && GetZoneID() != ZONE_BORDER_DEFENSE_WAR)
							{
...
bool Unit::isInAttackRange(Unit * pTarget, _MAGIC_TABLE * pSkill /*= nullptr*/)
{
	...
	const float fBaseMeleeRange		= 15.0f; // far too generous sek
	const float fBaseRangedRange	= 0.0f;
	...
	//uzaktan vurma olayfix 
	// Regular attack range.
	if (fWeaponRange != 0.02)
...
bool CUser::CanUseItem(uint32 nItemID, uint32 sCount /*= 1*/)
{
	...
	if (this == nullptr)//visualdev
		return false;

	/*if (GetID() > NPC_BAND) //visualdev
		return false;*/

	// Disable scroll usage while transformed.//visualdev
...
void CUser::ExpEvent(int64 exp)
{
	...
	if(rand > 0 && rand < g_pMain->m_byExpx2)
		...
		rand = 500;
	else if(rand >= g_pMain->m_byExpx500 && rand < (g_pMain->m_byExpx1000+1))
		rand = 1000;
...
bool CUser::UpdateKC(uint32 nAmount)
{
	g_DBAgent.UpdateAccountKnightCash(GetAccountName(), nAmount);
	return true;
}
void CUser::UpdateStatusCSW()
{
	...
	Packet result(WIZ_ITEM_MOVE, command);
		...
		result << command
			<< uint8(m_sTotalHit * m_bAttackAmount / 100) 
			<< uint16(m_sTotalAc + m_sACAmount)
			<< m_sMaxWeight
			<< m_iMaxHp << m_iMaxMp
			<< GetStatBonusTotal(STAT_STR) << GetStatBonusTotal(STAT_STA)
