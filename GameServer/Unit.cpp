...
float Unit::GetDistance(Unit * pTarget)
{
	...
	if (m_bZone && GetZoneID() != pTarget->GetZoneID())
		return -FLT_MAX;
...
void Unit::InsertRegion(int16 insert_x, int16 insert_z)
{
	...
	g_pMain->Send_NewRegions(&result, insert_x, insert_z, GetMap(), GetRegionX(), GetRegionZ(), nullptr, GetEventRoom());//error
}
#endif
...
{
	if (pTarget->isDead() || pTarget->isDead() || pTarget-> isBlinking())
		return 0;
	...
	if (pTarget)
	{
		if (pTarget->isNPC())
		{
			...
			if (pNpc != nullptr)
			{
				...
				if (pNpc->GetType() == NPC_BDW_MONUMENT)
					return true;
...
bool CUser::isHostileTo(Unit * pTarget)
{
	...
	if (isInSafetyArea() || (pTarget->isPlayer() && TO_USER(pTarget)->isInSafetyArea()))
		return false;
	...
	if (isInTempleEventZone() && (pTarget->GetNation() != GetNation() || !isSameEventRoom(pTarget)))
		return true;
	...
	if (g_pMain->m_byBattleSiegeWarOpen && GetZoneID() == ZONE_DELOS)
	{
		...
		if(pUser == nullptr || pTargetUser == nullptr)
		return false;
		if (pUser->GetClanID() > 0 && pTargetUser->GetClanID() > 0){
...
bool CUser::isInSafetyArea()
{
	...
	case ZONE_BATTLE4:
		if (GetNation() == KARUS)
			return ((GetX() < 300.0f && GetX() > 106.0f) && ((GetZ() < 251.0f && GetZ() > 199.0f)));
		else if (GetNation() == ELMORAD)
			return ((GetX() < 844.0f && GetX() > 741.0f) && ((GetZ() < 821.0f && GetZ() > 730.0f)));
	case ZONE_DELOS:
