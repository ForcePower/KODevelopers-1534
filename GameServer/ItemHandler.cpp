...
void CUser::WarehouseProcess(Packet & pkt)
{
	...
	if (isMerchanting())
		goto fail_return;// pus acýkken inn açmama kontrolü yok kanka
...
bool CUser::RobItem(uint32 nItemID, uint32 sCount /*= 1*/)
{
	...
	std::string errorMessage = string_format(_T("ROB_ITEM uId-%s- I-%d- C-%d- Z-%d- X-%d- Y-%d-"),
		GetName().c_str(),nItemID,sCount,GetZoneID(), uint16(GetX()), uint16(GetZ()));
...
bool CUser::RobItem(uint8 bPos, _ITEM_TABLE * pTable, uint32 sCount /*= 1*/)
{
	...
	if (pItem->nNum != pTable->m_iNum || pItem->sCount < sCount)
...
void CUser::ItemMove(Packet & pkt)
{
	...
	if (isTrading() 
		...
		|| m_bMerchantStatex == 1)// bu tarz kontrollere bunu ekle= ?gerek yok tmm
		goto fail_return;
	...
	case ITEM_COSP_TO_INVEN:
		...
		_ITEM_DATA * Try;
		Try = &m_sItemArray[SLOT_MAX + bDstPos];
		if (Try->nNum > 0)
			goto fail_return;
...
bool CUser::CheckExchange(int nExchangeID)
{
	...
	// Chaotic generator agýrlýk bugu
...
bool CUser::RunExchange(int nExchangeID, uint32 count)
{
	...
	if (pExchange->bRandomFlag == 101 
		&& !CheckExistItem(pExchange->nOriginItemNum[0],temp_sOriginItemCount0))
		return false;
	...
	else if (pExchange->bRandomFlag == 101)
	{
		...
		{
			sUpgradeNotice = string_format("%s has obtained of Moira '%s'.",GetName().c_str(),pItem->m_sName.c_str());
			g_pMain->SendUpgradeRebirth<PUBLIC_CHAT>(sUpgradeNotice.c_str());
		}
		///
	}
