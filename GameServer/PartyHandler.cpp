...
void CUser::PartyCancel()
{
	...
	_PARTY_GROUP *pParty = g_pMain->GetPartyPtr(GetRequestPartyID());
	if (pParty == nullptr)
		return;
...
void CUser::PartyRequest(int memberid, bool bCreate)
{
	...
	if (pUser == nullptr
		|| pUser == this
		|| pUser->isInParty()
		|| pUser->m_sPartyRequest != uint16(-1))
		goto fail_return;
	...
	if(GetZoneID() == ZONE_DELOS && (isInClan() && !pUser->isInClan()))
	{
		errorCode = -3;
		goto fail_return;
	}else if(GetZoneID() == ZONE_DELOS && (GetClanID() != pUser->GetClanID()))
	{
		errorCode = -3;
		goto fail_return;
	}else if(GetZoneID() == ZONE_DELOS && (!isInClan() && pUser->isInClan()))
	{
		errorCode = -3;
		goto fail_return;
	}
...
void CUser::PartyInsert()
{
	...
	pParty = g_pMain->GetPartyPtr(GetRequestPartyID());
	if (pParty == nullptr)
	{
		m_bInParty = false;
		m_sPartyIndex = -1;
...
void CUser::PartyRemove(int memberid)
{
	...
	if (pParty == nullptr) 
	{
		...
		m_sPartyRequest = pUser->m_sPartyRequest = -1;
