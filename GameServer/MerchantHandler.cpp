...
void CUser::MerchantOpen()
{
	...
	//else if (GetZoneID() > ZONE_MORADON || ZONE_ELMORAD || ZONE_KARUS || GetZoneID() <= ELMORAD)
	...
	{
	Packet resultmer;
	std::string bufferpro = string_format("[Merchant Message] must be at least 20 level"); 
	...
	{
	Packet resultmer;
	std::string bufferpro = string_format("[Merchant Message] only can trade in moradon zone"); 
	ChatPacket::Construct(&resultmer, 7, &bufferpro);
	Send(&resultmer);
	m_bMerchantStatex = 0;
	}
if (GetLevel() > 19 && GetZoneID() == ZONE_MORADON)  
	{
	Packet result(WIZ_MERCHANT, uint8(MERCHANT_OPEN));
	result << errorCode;
	Send(&result);
	// bura aciyor merchantý kanka
	}
...
void CUser::MerchantItemAdd(Packet & pkt)
{
	...
	if	(nItemID >= ITEM_NO_TRADE // Cannot be traded, sold or stored.
...
void CUser::MerchantItemBuy(Packet & pkt)
{
	...
	string errorMessage = string_format(_T("MERCHANT_ITEM_BUY buyerId-%s- sellerId-%s- Gold-%d- I-%d-, C-%d-"),
		GetName().c_str(),pMerchant->GetName().c_str(), req_gold, itemid, item_count);
		g_pMain->WriteMerchantUserLogFile(errorMessage);
...
void CUser::MerchantInsert(Packet & pkt)
{
	...
	string errorMessage = string_format(_T("MERCHANT_INSERT Msg-%s- uId-%s- %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d"),
		advertMessage.c_str(),GetName().c_str(),m_arMerchantItems[0].nNum,m_arMerchantItems[1].nNum,m_arMerchantItems[2].nNum,m_arMerchantItems[3].nNum,
		m_arMerchantItems[4].nNum,m_arMerchantItems[5].nNum,m_arMerchantItems[6].nNum,m_arMerchantItems[7].nNum,m_arMerchantItems[8].nNum,m_arMerchantItems[9].nNum,
		m_arMerchantItems[10].nNum,m_arMerchantItems[11].nNum);
...
void CUser::BuyingMerchantOpen(Packet & pkt)// buy varmý ki kanka yüksek versiondan düþürmeya fileslar silelim gitsin
{
	int16 errorCode = 0;
	if (isDead())
		errorCode = MERCHANT_OPEN_DEAD;
