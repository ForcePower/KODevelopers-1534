...
void CUser::SaveEvent(uint16 sQuestID, uint8 bQuestState)
{
	...
	if (CheckExistEvent(sQuestID, 2) && (bQuestState ==2 || bQuestState ==1 || bQuestState ==4)){
	Packet resultmer;
...
bool CUser::QuestV2RunEvent(_QUEST_HELPER * pQuestHelper, uint32 nEventID, int8 bSelectedReward /*= -1*/)
{
	...
	result = g_pMain->GetLuaEngine()->ExecuteScript(this, pNpc, nEventID, bSelectedReward, pQuestHelper->strLuaFilename.c_str());//error 2
