...
void CUser::Attack(Packet & pkt)
{
	...
	if (isIncapacitated()) // debug tamý acayým bundan sonra evet okey
	{
		return;
	}
	...
	if (pTarget != nullptr && pTarget->isPlayer() && (TO_USER(pTarget)->isInSafetyArea() || isInSafetyArea()))
	{
		return;
	}
		...
		&& CanAttack(pTarget))
	{
		...
		{
			if (isInTempleEventZone() && (!isSameEventRoom(pTarget) || !g_pMain->pTempleEvent.isAttackable[GetEventRoom()] ))
			{
				return;
			}
...
void CUser::Regene(uint8 regene_type, uint32 magicid /*= 0*/)
{
	...
	if (magicid == 0 && GetZoneID() != ZONE_RONARK_LAND)
		BlinkStart();
