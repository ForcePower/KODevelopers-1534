...
int8 CDBAgent::AccountLogin(string & strAccountID, string & strPasswd)
{
	...
	if (!dbCommand->Execute(string_format(_T("{? = CALL GAME_LOGIN(?, ?)}"))))
		ReportSQLError(m_GameDB->GetError());
...
void CDBAgent::BifrostBosalt()
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return;

	if (!dbCommand->Execute(_T("UPDATE USERDATA SET ZONE=71 WHERE ZONE=31")))
		ReportSQLError(m_GameDB->GetError());

	printf("Bifrost bosaltildi..\n");
}
void CDBAgent::karusbackt()
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return;

	if (!dbCommand->Execute(_T("update USERDATA set Zone=21,PX=31000,PZ=36000,PY=0 where Nation=1 and Zone in (61,62,63,64)")))
		ReportSQLError(m_GameDB->GetError());

	printf("Karus back..\n");
}
void CDBAgent::moradbackt()
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return;

	if (!dbCommand->Execute(_T("update USERDATA set Zone=21,PX=31000,PZ=36000,PY=0 where Nation=2 and Zone in (61,62,63,64)")))
		ReportSQLError(m_GameDB->GetError());
...
void CDBAgent::UpdateUserHWBAN(CUser *strUserID)
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return;

	dbCommand->AddParameter(SQL_PARAM_INPUT, strUserID->GetAccountName().c_str(), strUserID->GetAccountName().length());
	if (!dbCommand->Execute(string_format(_T("insert into Hardwarebanlist values(?,'" I64FMTD "',getdate())"), strUserID->nHwID)))
		ReportSQLError(m_GameDB->GetError());
...
uint8 CDBAgent::GetUnreadLetterCount(string & strCharID)
{
	...
	dbCommand->AddParameter(SQL_PARAM_INPUT, strCharID.c_str(), strCharID.length());
	if (!dbCommand->Execute(_T("{? = CALL MAIL_BOX_CHECK_COUNT(?)}")))// bu prosedüre baksana
	{
		ReportSQLError(m_GameDB->GetError());
		return 0;
	}
...
void CDBAgent::UpdateAccountKnightCash(string & strAccountID, uint32 KnightCash)
{
	...
	DateTime time;
	string errorMessage = string_format(_T("[ KC Exchange - %d.%d.%d %d:%d:%d ] strAccountID: %s KnightCash: %d\n"),
		time.GetDay(),time.GetMonth(),time.GetYear(),time.GetHour(),time.GetMinute(),time.GetSecond(),strAccountID.c_str(),KnightCash);

	g_pMain->m_fpKnightCash = fopen(string_format("./Logs/KnightCash%d_%d_%d.log",time.GetDay(),time.GetMonth(),time.GetYear()).c_str(), "a");
	if (g_pMain->m_fpKnightCash != nullptr)
	{
		fwrite(errorMessage.c_str(), errorMessage.length(), 1, g_pMain->m_fpKnightCash);
		fclose(g_pMain->m_fpKnightCash);
	}
...
void CDBAgent::AccountBanned(string & strUserID )
{
	unique_ptr<OdbcCommand> dbCommand(m_AccountDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return;
	...
	dbCommand->AddParameter(SQL_PARAM_INPUT, strUserID.c_str(), strUserID.length());
	if (!dbCommand->Execute(_T("{CALL ACCOUNT_BANNED(?)}")))
		ReportSQLError(m_AccountDB->GetError());
}
void CDBAgent::AccountUnbanned(string & strUserID)
{
	unique_ptr<OdbcCommand> dbCommand(m_AccountDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return;
	...
	dbCommand->AddParameter(SQL_PARAM_INPUT, strUserID.c_str(), strUserID.length());
	if (!dbCommand->Execute(_T("{CALL ACCOUNT_UNBANNED(?)}")))
		ReportSQLError(m_AccountDB->GetError());
}
void CDBAgent::AccountIpBanned(string & strUserID )
{
	unique_ptr<OdbcCommand> dbCommand(m_AccountDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return;
	...
	dbCommand->AddParameter(SQL_PARAM_INPUT, strUserID.c_str(), strUserID.length());
	if (!dbCommand->Execute(_T("{CALL ACCOUNT_IPBANNED(?)}")))
		ReportSQLError(m_AccountDB->GetError());
}

void CDBAgent::ItemOnline(uint32 nItemID ,uint16 sCount)
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return;

	if (!dbCommand->Execute(string_format(_T("{CALL SEND_ITEM(%d,%d)}"), nItemID,sCount)))
		ReportSQLError(m_GameDB->GetError());
}