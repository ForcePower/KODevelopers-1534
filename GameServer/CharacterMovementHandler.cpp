...
bool CUser::isInBowl(uint16 X, uint16 Y)
{
	if (X > 9660 
		&& X < 11100
		&& Y > 8610
...
void CUser::MoveProcess(Packet & pkt)
{
	...
	{
		if(isInBowl(GetSPosX(),GetSPosZ()) && isInBowl(will_x,will_z) && !inBowl)
		{
			inBowl = true;
			...
			// Welcome to BOWL
			Packet resultmer;
			std::string bufferpro = string_format("[Event Message] Welcome to Ronark land bowl!"); 
			ChatPacket::Construct(&resultmer, 7, &bufferpro);
			Send(&resultmer);
		}
		else if (!isInBowl(GetSPosX(),GetSPosZ()) && isInBowl(will_x,will_z))
		{
			inBowl = true;
			BowlRewardTime = UNIXTIME;
			lastPrize = 0;
		...
		}else if (isInBowl(GetSPosX(),GetSPosZ()) && !isInBowl(will_x,will_z) && inBowl)
		{
			inBowl = false;
			// GG from BOWL
			Packet resultmer;
			std::string bufferpro = string_format("[Event Message] You have leaved Ronark land bowl!"); 
			ChatPacket::Construct(&resultmer, 7, &bufferpro);
			Send(&resultmer);
		}}
...
void CUser::GetUserInfo(Packet & pkt)
{
	...
	else
	{
		...
		pkt	<< pKnights->GetAllianceID()
			...
			<< pKnights->GetCapeID(aKnights); // cape ID  /error
	}
...
bool CUser::CanChangeZone(C3DMap * pTargetMap, WarpListResponse & errorReason)
{
	...
	case ZONE_DELOS: // TODO: implement CSW logic.
		if (g_pMain->m_byBattleOpen == CLAN_BATTLE && !g_pMain->m_byBattleSiegeWarTeleport || /*(*/ g_pMain->m_byBattleOpen == CLAN_BATTLE && GetClanID() <= 0/* && (GetClanID() > 0 && GetClanGrade() > 3)*/)/*)*/
		{
	...
	case ZONE_BIFROST:
		return true;
	case ZONE_ARDREAM:
		if (g_pMain->isWarOpen() && g_pMain->m_byBattleOpen != CLAN_BATTLE && g_pMain->m_byBattleZoneType != ZONE_ARDREAM)
		{
	...
	case ZONE_RONARK_LAND_BASE:
		if (g_pMain->isWarOpen() && g_pMain->m_byBattleZoneType != ZONE_ARDREAM && g_pMain->m_byBattleOpen != CLAN_BATTLE)
		{
			errorReason = WarpListNotDuringWar;
			return false;
		}
...
void CUser::ZoneChange(uint16 sNewZone, float x, float z)
{
	...
	if (GetZoneID() != sNewZone)
	{
		...
		m_bZoneChangeSameZone = false; //error 
	...
	if(isBlinking())
	{
		...
		Packet resultt(AG_USER_REGENE);
		resultt	<< GetSocketID() << m_sHp;
		Send_AIServer(&result);

		resultt.Initialize(AG_USER_INOUT);
		resultt.SByte(); // TODO: Remove this redundant uselessness that is mgame
		resultt	<< uint8(INOUT_RESPAWN) << GetSocketID()
			<< GetName()
			<< GetX() << GetZ();
		Send_AIServer(&resultt);
...
{
	if (g_pMain->pTempleEvent.ActiveEvent == TEMPLE_EVENT_BORDER_DEFENCE_WAR && !isEventUser())
	{
		C3DMap * pMap = g_pMain->GetZoneByID(ZONE_BORDER_DEFENSE_WAR);
		...
		if ((g_pMain->pTempleEvent.KarusUserCount+g_pMain->pTempleEvent.ElMoradUserCount)%5==0){
			g_pMain->Send_All(&resultmer,nullptr,Nation::ALL,0,true);
		}else{
			Send(&resultmer);
		}
	}
...
void CUser::Warp(uint16 sPosX, uint16 sPosZ)
{
	...
	if(g_pMain->CZBowlAktifMi)
	{
		if(isInBowl(GetSPosX(),GetSPosZ()) && isInBowl(sPosX,sPosZ) && !inBowl)
		{
			inBowl = true;
			BowlRewardTime = UNIXTIME;
			lastPrize = 0;
		...
		else if (!isInBowl(GetSPosX(),GetSPosZ()) && isInBowl(sPosX,sPosZ))
		{
			inBowl = true;
			BowlRewardTime = UNIXTIME;
			lastPrize = 0;
		...
		}else if (isInBowl(GetSPosX(),GetSPosZ()) && !isInBowl(sPosX,sPosZ) && inBowl)
		{
			inBowl = false;
			// GG from BOWL
			Packet resultmer;
...
void CUser::RecvZoneChange(Packet & pkt)
{
	...
	else if (opcode == ZoneChangeLoaded)
	{
		...
		if (!m_bZoneChangeSameZone && !(GetZoneID() == ZONE_RONARK_LAND
