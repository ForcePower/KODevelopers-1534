...
uint32 THREADCALL DatabaseThread::ThreadProc(void * lpParam)
{
	while (true)
	{
		...
		case WIZ_KNIGHTS_PROCESS:
			CKnightsManager::ReqKnightsPacket(pUser, pkt); //error clan
			break;
		case WIZ_LOGIN_INFO:
		...
		case WIZ_SHOPPING_MALL:
			...
			break;// kanka buranın calıstıgını bılıyoruz burdan sonra bozuluyor bence
		case WIZ_SKILLDATA:
...
void CUser::ReqCreateNewChar(Packet & pkt)
{
	...
	/*if (!WordGuardSystem(strCharID,strCharID.length())){
	hatali=3;}

	if (bStr < 50  || bSta < 50 || bDex < 50 || bInt < 50 || bCha < 50){
	hatali=3;}*/
...
void CUser::ReqSelectCharacter(Packet & pkt)
{
	...
	if ((g_pMain->m_byBattleOpen == CLAN_BATTLE && GetZoneID() == ZONE_DELOS)
		||
		(GetZoneID() == ZONE_BIFROST &&
		g_pMain->m_BifrostVictory != GetNation())){
			NativeZoneReturn();
	...
	if ((((GetZoneID() != GetNation() && GetZoneID() <= ZONE_ELMORAD && !g_pMain->m_byBattleOpen)
		// also disallow players from logging back into war zones that aren't currently active...
			|| ((GetZoneID() > 60 && GetZoneID() < 70) && !g_pMain->m_byBattleOpen)
			...
			|| ((GetZoneID() == ZONE_ELMORAD_ESLANT || GetZoneID() == ZONE_KARUS_ESLANT) && (GetZoneID() - 10) != GetNation() ))
	{
		NativeZoneReturn();
		g_DBAgent.UpdateUser(GetName(), UPDATE_LOGOUT, this);
		Disconnect();
...
void CUser::ReqSaveCharacter()
{
	if (m_bLevel == 0 || m_bNation){
		TRACE("### ReqSaveCharacter - Level is Zero : bRoom=%d, bNation=%d, bZone=%d, Level=%d ####\n", GetEventRoom(), GetNation(), GetZoneID(),m_bLevel);
...
void CKnightsManager::ReqKnightsPacket(CUser* pUser, Packet & pkt)
{
	...
	case KNIGHTS_WITHDRAW:
		ReqUpdateKnights(pUser, pkt, opcode);//error clan
		break;
	case KNIGHTS_REMOVE:
	case KNIGHTS_ADMIT:
	case KNIGHTS_REJECT:
...
void CKnightsManager::ReqKnightsAllianceCreate(CUser *pUser, Packet & pkt) {
	...
	uint16 UserClanID = pUser->m_bKnights;//prueba 1


	CKnights *pKnights = g_pMain->GetClanPtr(pUser->GetClanID());
	uint8 byType = 44, byEmptyIndex = 0, bySiegeFlag = pKnights->bySiegeFlag;

	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KNIGHTS_ALLY_CREATE));
	//g_DBAgent.CreateAlliance(byType, pUser->GetClanID(), 0, 0, bySiegeFlag);
	g_DBAgent.CreateAlliance(byType, UserClanID, 0, 0, bySiegeFlag);//prueba 1

	_KNIGHTS_ALLIANCE *pAlliance = new _KNIGHTS_ALLIANCE();

	//pAlliance->sMainAllianceKnights = pUser->GetClanID();
	pAlliance->sMainAllianceKnights = UserClanID;//prueba 1
	...
	result << uint8(1) << UserClanID;//prueba 1
	if(pUser->isInGame())//prueba 1
		pUser->Send(&result);//prueba 1
...
void CKnightsManager::ReqKnightsAllianceRequest(CUser *pUser, Packet & pkt) {
	...
	pAlliance->sMainAllianceKnights = MainClanID;
	g_pMain->m_KnightsAllianceArray.PutData(pAlliance->sMainAllianceKnights, pAlliance);
	result << Type << MainClanID << TargetClanID << pMainClan->m_sCape;
...
void CKnightsManager::ReqKnightsAllianceRemove(CUser *pUser, Packet & pkt) {
	...
	if(pTargetClan == nullptr || pMainClan == nullptr)
		return;
	_KNIGHTS_ALLIANCE * pAlliance = g_pMain->GetAlliancePtr(MainClanID);
	...
	if (pAlliance != nullptr) {
		...
		result << uint8(1) << MainClanID << TargetClanID  << pTargetClan->m_sCape;
		pTargetClan->SendToRegion(&result);
	}
}
...
void CKnightsManager::ReqUpdateKnights(CUser *pUser, Packet & pkt, uint8 opcode)
{
	...
	RecvUpdateKnights(pUser, result, opcode);//error clan
}
