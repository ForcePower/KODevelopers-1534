...
void CUser::ExchangeReq(Packet & pkt)
{
	...
	{
		Packet resultmer;
		std::string bufferpro = string_format("[NPC Message] You cannot do anything when you trading."); 
	...
	else
	{
		...
		else if (isTrading() || m_bMerchantStatex == 1)
		{
			ExchangeCancel();
			return;
		}
		...
		if (pUser == nullptr
			|| pUser->isTrading()
			|| pUser->m_bMerchantStatex == 1
			|| pUser->GetZoneID() != GetZoneID()
			|| (GetNation() != GetNation() && GetMap()->canTradeWithOtherNation()))
			goto fail_return;

		m_sExchangeUser = destid;//tamam kanka eyw burdan biþey çýkmaz sanmýyorum
		pUser->m_sExchangeUser = GetSocketID();
		if (pUser->isDead() || pUser->isStoreOpen() || pUser->isMerchanting() || pUser->m_bMerchantStatex == 1 || pUser == nullptr || !pUser->isInGame())
			goto fail_return;

		if(pUser->GetID() == GetID()) {
			m_bAuthority = AUTHORITY_BANNED;
			Disconnect();
			std::string BanNotice;
			BanNotice=string_format("%s is currently blocked for illegal activity. Reason: Tried to do trade bug.", GetName().c_str());
			g_pMain->SendNotice(BanNotice.c_str(),Nation::ALL);

			printf("%s is currently blocked for illegal activity. Trade BUG \n",GetName().c_str());
			return;
		}
...
void CUser::ExchangeAgree(Packet & pkt)
{
	...
	if (pUser->isDead() || pUser->isStoreOpen() || pUser->isMerchanting() || pUser->m_bMerchantStatex)
	{
		m_sExchangeUser = -1;
		return;
	}
...
void CUser::ExchangeDecide()
{
	...
	{
		if(!isTrading() || !pUser->isTrading() || m_bMerchantStatex == 1 || pUser->m_bMerchantStatex == 1){
			ExchangeCancel();
			pUser->ExchangeCancel();
			printf ("%s is currently disconnected for illegal activity. Reason: Tried to do trade bug.\n", GetName().c_str());
		}else{
			ExecuteExchange();
...
bool CUser::ExecuteExchange()
{
	...
	foreach (Iter, pUser->m_ExchangeItemList)
	{
		...
		pDstItem->sCount = pDstItem->sDuration;  // Kesinlikle yanlis
