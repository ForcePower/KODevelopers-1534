...
#define NO_ACTION				0
#define ATTACK_TO_TRACE			1				// °ø°Ý¿¡¼­ Ãß°Ý
#define LONG_ATTACK_RANGE		30				// Àå°Å¸® °ø°Ý À¯È¿°Å¸®
#define SHORT_ATTACK_RANGE		1			// Á÷Á¢°ø°Ý À¯È¿°Å¸®
...
bool CNpc::RegisterRegion(float x, float z, int id)
{
 MAP* pMap = GetMap(); 
 if (pMap == nullptr) 
 {
...
void CNpc::Load(uint16 sNpcID, CNpcTable * proto, bool bMonster, uint8 nation, uint16 nEventRoom)
{
	...
	m_sAttackDelay		= proto->m_sAttackDelay / 2;
	m_sSpeed			= proto->m_sSpeed / 4;
	
	m_DamagedUserListCount = 0;		// dmgx
	...
	m_fSpeed_1			= ((float)proto->m_bySpeed_1 * ((float)250 / 1000));
	m_fSpeed_2			= ((float)proto->m_bySpeed_2 * ((float)250 / 1000));
	m_fOldSpeed_1		= ((float)proto->m_bySpeed_1 * ((float)250 / 1000));
	m_fOldSpeed_2		= ((float)proto->m_bySpeed_2 * ((float)250 / 1000));
	...
	m_bySearchRange		= proto->m_bySearchRange / 2;
	m_byAttackRange		= uint8(proto->m_byAttackRange / 1.5);
	m_byTracingRange	= proto->m_byTracingRange / 2;
...
void CNpc::SendMoveResult(float fX, float fY, float fZ, float fSpeed /*= 0.0f*/)
{
	...
	g_pMain->Send(&result);
	RegisterRegion(fX, fZ, m_sNid + NPC_BAND);
}
...
time_t CNpc::NpcTracing()
{
	...
	auto result = IsCloseTarget(int(m_byAttackRange / 1.5), AttackTypePhysical);
	...
	if (m_bStopFollowingTarget)
	{
		...
		{
			TRACE("##### NpcTracing Fail : ÆÐ½ºÆÄÀÎµå ½ÇÆÐ , NPC_STANDINGÀ¸·Î ######\n");
			InitTarget();
			NpcMoveEnd();	// ÀÌµ¿ ³¡..
			m_NpcState = NPC_STANDING;
			return m_sStandTime;
	...
	{
		m_NpcState = NPC_STANDING;
		TRACE("### NpcTracing Fail : StepMove ½ÇÆÐ, %s, %d ### \n", GetName().c_str(), GetID());
		return m_sStandTime;
	}
...

{
	...
	auto result = IsCloseTarget(int(m_byAttackRange / 1.5), AttackTypeNone);
	if(!isMonster() && GetProto()->m_byDirectAttack == 2)
		result = IsCloseTarget(int(m_byAttackRange), AttackTypeMagic);
	...
	else if (nValue == 0)
	{
		...
		//if (GetName() == "Werewolf" && GetZoneID() == ZONE_MORADON)
	//printf("#### %d m_fSecForMetor = %g m_fSpeed_2 %g\n", GetID(), m_fSecForMetor,m_fSpeed_2);
...
time_t CNpc::NpcMoving()
{
	...
	SendMoveResult(m_fPrevX, m_fPrevY, m_fPrevZ, (float)(m_fSecForRealMoveMetor));
	return m_sSpeed;	
}
...
time_t CNpc::NpcStanding()
{
	...
	if (GetType() == NPC_SPECIAL_GATE 
		&& g_pMain->m_byBattleEvent == BATTLEZONE_OPEN && GetZoneID() != ZONE_DELOS )
	{
...
time_t CNpc::NpcBack()
{
		...
		&& g_pMain->GetUnitPtr(m_Target.id) == nullptr)
	{
		m_NpcState = NPC_STANDING;
		if (isMonster()){
		//printf ("%s monster, NpcBack4 some one. m_bSpeedAmount: %d.\n",GetName().c_str(),m_bSpeedAmount);
	}
		return m_sSpeed ;
	//STEP_DELAY;
	}
	...
	if (isMonster()){
		printf ("%s monster, NpcBack5 some one. m_bSpeedAmount: %d.\n",GetName().c_str(),m_bSpeedAmount);
	}
	return m_sSpeed ;
...
bool CNpc::SetLive()
{
	...
	RegisterRegion(GetX(), GetZ(),m_sNid + NPC_BAND);
...
bool CNpc::RandomMove()
{
	...
	if (m_bSpeedAmount < 100)
		m_fSecForMetor = m_fSpeed_1 * float(100 / (100 + m_bSpeedAmount));
		else
		m_fSecForMetor = m_fSpeed_1;

	//if (GetName() == "Werewolf" && GetZoneID() == ZONE_MORADON)
//printf("#### %d m_fSecForMetor = %g m_fSpeed_1 %g\n", GetID(), m_fSecForMetor,m_fSpeed_1);
...
// regionÀ» 4µîºÐÇØ¼­ ¸ó½ºÅÍÀÇ ÇöÀç À§Ä¡°¡ regionÀÇ ¾î´À ºÎºÐ¿¡ µé¾î°¡´ÂÁö¸¦ ÆÇ´Ü
int CNpc::GetMyField()
{
	int iRet = 0;
...
int CNpc::IsSurround(CUser* pUser)
{
	...
	if(pUser == nullptr)	return -2;		// User°¡ ¾øÀ¸¹Ç·Î Å¸°ÙÁöÁ¤ ½ÇÆÐ..
...
bool CNpc::StepMove()
{
	...
	if(m_pPoint[m_iAniFrameCount].fXPos < 0 || m_pPoint[m_iAniFrameCount].fZPos < 0)
	{
		...
		RegisterRegion(m_fPrevX, m_fPrevZ,m_sNid + NPC_BAND);
		return false;	
	}
	...
	if (m_sStepCount++ > 0)
	{
		...
		return RegisterRegion(GetX(), GetZ(),m_sNid + NPC_BAND);
	}
...
//printf("#### %d m_fSecForMetor = %g m_fSpeed_2 %g\n", GetID(), m_fSecForMetor,m_fSpeed_2);
	...
	if (m_Target.id < NPC_BAND)	
	{
		...
		if(option == 1)	{	// magicÀÌ³ª È°µîÀ¸·Î °ø°Ý ´çÇß´Ù¸é...
			vNpc.Set(GetX(), GetY(), GetZ());
			vUser.Set(pUser->GetX(), pUser->GetY(), pUser->GetZ()); 
			...
			if(IsDamagedUserList(pUser)) iTempRange = (float)(m_byTracingRange * 2);	// °ø°Ý¹ÞÀº »óÅÂ¸é Ã£À» ¹üÀ§ Áõ°¡.
			else iTempRange += 2;
		}
...
time_t CNpc::Attack()
{
	...
	if (nID < NPC_BAND)	
	{
		...
		if (sqrt(pow((pUser->m_curx - GetX()), 2) + pow((pUser->m_curz - GetZ()), 2)) < m_bySearchRange)
...
{
	int nStandingTime = m_sStandTime;
	auto result = IsCloseTarget(int(m_byAttackRange / 1.5), AttackTypeMagic);
	if (result == CloseTargetNotInRange)	
	{
		m_sStepCount = 0;
		m_byActionFlag = ATTACK_TO_TRACE;
...
if (preUser != nullptr)
	...
	else if(preUser == nullptr && nAttackType == 1004)		return;		// Heal magic¿¡ ¹ÝÀÀÇÏÁö ¾Êµµ·Ï..
	...
	{									// °¡±îÀÌ ÀÖÀ¸¸é ¹Ý°ÝÀ¸·Î ÀÌ¾îÁö±¸
		...
		else							// ¹Ù·Î µµ¸Á°¡¸é ÁÂÇ¥¸¦ °»½ÅÇÏ°í ÃßÀû	
		{
			...
			if(nValue == 1)	// ¹Ý°Ý µ¿ÀÛÈÄ ¾à°£ÀÇ µô·¹ÀÌ ½Ã°£ÀÌ ÀÖÀ½	
			{
				m_NpcState = NPC_TRACING;
				m_Delay = 0;
			}
...
//	³ª¸¦ °ø°ÝÇÑ Npc¸¦ Å¸°ÙÀ¸·Î »ï´Â´Ù.(±âÁØ : ·¾°ú HP¸¦ ±âÁØÀ¸·Î ¼±Á¤)
void CNpc::ChangeNTarget(CNpc *pNpc)
{
	...
	if (m_NpcState == NPC_STANDING || m_NpcState == NPC_MOVING || m_NpcState == NPC_SLEEPING)
	{
		if (IsCloseTarget(int(m_byAttackRange / 1.5), AttackTypeNone) == 1)
		{
			m_NpcState = NPC_FIGHTING;
			m_Delay = 0;
		}
...
{
	for (int i = 0; i < NPC_HAVE_USER_LIST; i++){
		if ((UNIXTIME - m_DamagedUserList[i].lastdamagedt) > 60 && m_DamagedUserList[i].GetID != sAttackerID && m_DamagedUserList[i].Damage > 0)
			m_DamagedUserList[i].GetID = -1;
	}
	...
	if (IsDamagedUserList(pAttacker)){
		for (int i = 0; i < NPC_HAVE_USER_LIST; i++){
			if (m_DamagedUserList[i].GetID == pAttacker->GetID()){
				...
				m_DamagedUserList[i].lastdamagedt = UNIXTIME;
			}
		}
	}else{
		if(m_DamagedUserListCount < 100){
		m_DamagedUserList[m_DamagedUserListCount].GetID = sAttackerID;
		m_DamagedUserList[m_DamagedUserListCount].Damage = MyDamage;
		m_DamagedUserList[m_DamagedUserListCount].lastdamagedt = UNIXTIME;
...
void CNpc::SendExpToUserList()
{
	...
	for (int i = 0; i < NPC_HAVE_USER_LIST; i++){
		...
		if ((UNIXTIME - m_DamagedUserList[i].lastdamagedt) > 60)
			continue;
...
void CNpc::FindFriendRegion(int x, int z, MAP* pMap, _TargetHealer* pHealer, MonSearchType type)
{
	...
	__Vector3 vStart, vEnd;
	float fDis = 0.0f, 
		fSearchRange = (type == MonSearchNeedsHealing ? float((float)m_byAttackRange / 1.5) : (float)m_byTracingRange);
	int iValue = 0;
...
// ÆÐ½º ÆÄÀÎµå¸¦ ÇÏÁö ¾Ê°í °ø°Ý´ë»óÀ¸·Î °¡´Â ·çÆ¾..
void CNpc::IsNoPathFind(float fDistance)
{
	...
	fDis = GetDistance(vStart, vEnd);	
	if(fDis > NPC_MAX_MOVE_RANGE)	{						// 100¹ÌÅÍ º¸´Ù ³ÐÀ¸¸é ½ºÅÄµù»óÅÂ·Î..
		ClearPathFindData();
...
{
	if (isGuard() || GetName() == "Guard tower")
		return true;
...
int	CNpc::ItemProdution(int item_number)							// ¾ÆÀÌÅÛ Á¦ÀÛ
{
	...
	if( COMPARE( iRandom, 1, 4001) )	{			// ¹«±â±¸ ¾ÆÀÌÅÛ
		...
		iItemCode = iTemp1 * 100000;	// ·çÆÃºÐÆ÷Ç¥ ÂüÁ¶

		iRand3 = myrand(1, 10000);					// Á¾Á·(¿¤¸ð, Ä«·ç½º)
		...
		if(iRandom == -1)	{						// Àß¸øµÈ ¾ÆÀÌÅÛ »ý¼º½ÇÆÐ
			return 0;
		}
		iRand5 = iRandom * 10;
		...
		if( iTemp1 == 0 )	return 0;
		iItemCode = iTemp1 * 1000000;		// ·çÆÃºÐÆ÷Ç¥ ÂüÁ¶
		...
		if( m_byMaxDamagedNation == KARUS )	{		// Á¾Á·
			...
			if( COMPARE( iRandom, 0, 2000) )	{		
				...
				iRand2 = 60000000;
				iRand3 = 30000;							// ¸¶¹ý»ç°©¿ÊÀº ¸µÅ¬ Åõ¾Æ·º¸¸ °¡Áöµµ·Ï
			}
			...
			if( COMPARE( iRandom, 0, 3300) )	{		
				iRand2 = 0;	
				iItemKey = myrand(0, 10000);			// Àü»ç°©¿ÊÀº ¸ðµç Á¾Á·ÀÌ °¡Áü
				...
				iRand2 = 60000000;
				iItemKey = myrand(0, 10000);			// ¸¶¹ý»ç°©¿ÊÀº ³²ÀÚ¿Í ¿©ÀÚ¸¸ °¡Áü
				if( COMPARE( iItemKey, 0, 5000) )	iRand3 = 120000;
				else								iRand3 = 130000;
			}
		...
		iRand5 = iRandom * 10;
		iItemNumber = iDefault + iRand2 + iItemCode + iRand3 + iRand4 + iRand5 + iItemGrade;	// iItemGrade : ¾ÆÀÌÅÛ µî±Þ»ý¼ºÇ¥ Àû¿ë
		...
		iDefault = 300000000;
		iRandom = myrand(0, 10000);					// ¾Ç¼¼»ç¸® Á¾·ù°áÁ¤(±Í°í¸®, ¸ñ°ÉÀÌ, ¹ÝÁö, º§Æ®)
		...
		iRand3 = myrand(1, 10000);					// Á¾Á·(¿¤¸ð¶óµå, Ä«·ç½º)
