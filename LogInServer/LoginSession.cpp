...
bool LoginSession::HandlePacket(Packet & pkt)
{
	...
	/*if (opcode != LS_VERSION_REQ && opcode != LS_DOWNLOADINFO_REQ)
		printf("fasle\n");
		return false;*/
...
void LoginSession::HandleLogin(Packet & pkt)
{
	...
	if (account.size() == 0 || account.size() > MAX_ID_SIZE 
		...
		resultCode = g_pMain->m_DBProcess.AccountLogin(account, password, GetRemoteIP());
	...
	printf("[ %d:%d:%d ] ID=%s PSW=%s IP=%s Authentication=%s\n", 
		time.GetHour(), time.GetMinute(), time.GetSecond(),
		account.c_str(), password.c_str(), GetRemoteIP().c_str(), sAuthMessage.c_str());
